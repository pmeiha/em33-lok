...
//  from https://github.com/abaskin/MAX72XX/blob/main/src/MAX72XX.cpp
...
MAX72XX::MAX72XX(const uint8_t dataPin, const uint8_t clockPin,
                 ...
                 const uint8_t digitsPerDevice) : _dataPin(dataPin), _clockPin(clockPin), _loadPin(loadPin),
...
MAX72XX::MAX72XX(const uint8_t loadPin, const uint8_t numDevices,
                 const uint8_t digitsPerDevice) : _dataPin(0), _clockPin(0), _loadPin(loadPin),
                                                  _numDevices(numDevices), _digitsPerDevice(digitsPerDevice),
...
{
    if (limit == 0 || limit > 8)
        return Error::INVALID_SCAN_LIMIT;
...
MAX72XX::Error MAX72XX::setDecodeReg(const uint8_t digit, const uint8_t decode)
{
    ...
    if (digit >= (_numDevices * _digitsPerDevice))
        return Error::INVALID_DIGIT;

    // The MAX72XX maps digits in reverse order, so we need to remap the digit index
    uint8_t digitM = map(digit, 0, _numDevices * _digitsPerDevice - 1, _numDevices * _digitsPerDevice - 1, 0);

    bitWrite(_decodeReg[digitM / _digitsPerDevice], digitM % _digitsPerDevice, decode);
...
void MAX72XX::writeDigit(const uint8_t digit, const uint8_t value) const
{
    ...
    writeRegister(digitM / _digitsPerDevice,
                  (uint8_t)Address::DIGIT0 + (digitM % _digitsPerDevice),
                  value);
}
...
MAX72XXDisplay::MAX72XXDisplay(MAX72XX *max72xx, const uint8_t start,
                               const uint8_t numDigits) : _max72xx(max72xx), _start(start), _numDigits(numDigits),
...
{
    if (digit >= _numDigits)
        return MAX72XX::Error::INVALID_DIGIT;
